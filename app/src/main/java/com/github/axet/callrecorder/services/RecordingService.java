...
import com.github.axet.audiolibrary.BuildConfig;
...
import com.github.axet.callrecorder.activities.RecorderSettingsActivity;
...
public class RecordingService extends PersistentService implements SharedPreferences.OnSharedPreferenceChangeListener {
    ...
    public static boolean isEnabled(Context context) {
        ...
        if (!Storage.permitted(context, RecorderMainActivity.MUST))
            b = false;
        return b;
    }
    ...
    public void setPhone(String s, String c) {
        ...
        contact = "";
        contactId = "";
        if (Storage.permitted(this, RecorderSettingsActivity.CONTACTS)) {
    ...
    public void onStartCommand(Intent intent) {
        ...
        if (a == null) {
            ...
            RecorderMainActivity.startActivity(this);
        }
    }
    ...
    public Notification buildPersistent(Notification when) {
        PendingIntent main = PendingIntent.getActivity(this, 0, new Intent(this, RecorderMainActivity.class), PendingIntent.FLAG_UPDATE_CURRENT);
    ...
    public void updateIcon(boolean show) {
        boolean recording = thread != null;
        RecorderMainActivity.showProgress(RecordingService.this, show, phone, samplesTime / sampleRate, recording);
            ...
            public void run() {
                ...
                Runnable save = new Runnable() {
                    ...
                    public void run() {
                        ...
                        RecorderMainActivity.last(RecordingService.this);
                    ...
                    while (!interrupt.get()) {
                        ...
                        if (stableRefresh || diff >= samples) {
                            ...
                            if (samplesTimeCount > samplesTimeUpdate) {
                                ...
                                RecorderMainActivity.showProgress(RecordingService.this, true, phone, samplesTime / sampleRate, true);
                            }
                        }
                    }
    ...
    void encoding(final File in, final Uri uri, final Runnable done, final Success success) {
        ...
        final Runnable save = new Runnable() {
            ...
            public void run() {
                ...
                RecorderMainActivity.showProgress(RecordingService.this, false, phone, samplesTime / sampleRate, false);
        ...
        encoder.run(new Runnable() {
            ...
            public void run() {  // progress
                RecorderMainActivity.setProgress(RecordingService.this, encoder.getProgress());
            }
        }, new Runnable() {
            @Override
    ...
    void pauseButton() {
        if (thread != null)
            ...
            startRecording();
        RecorderMainActivity.showProgress(this, true, phone, samplesTime / sampleRate, thread != null);
    }
