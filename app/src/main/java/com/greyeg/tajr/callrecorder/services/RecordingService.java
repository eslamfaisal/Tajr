...
public class RecordingService extends PersistentService {
    ...
    public static boolean isEnabled(Context context) {
        ...
        if (!Storage.permitted(context, MainActivityRecording.MUST))
            b = false;
        return b;
    }
...
//        builder.setImageViewResource(R.id.notification_pause, recording ? R.drawable.ic_stop_black_24dp : R.drawable.ic_play_arrow_black_24dp);
    ...
    public Notification buildPersistent(Notification when) {
        PendingIntent main = PendingIntent.getActivity(this, 0, new Intent(this, MainActivityRecording.class), PendingIntent.FLAG_UPDATE_CURRENT);
    ...
    public void updateIcon(boolean show) {
        boolean recording = thread != null;
        MainActivityRecording.showProgress(RecordingService.this, show, phone, samplesTime / sampleRate, recording);
...
//        RecentCallActivity.startActivity(this, targetUri, true);
            ...
            public void run() {
                ...
                Runnable save = new Runnable() {
                    ...
                    public void run() {
                        ...
                        MainActivityRecording.last(RecordingService.this);
                    ...
                    while (!interrupt.get()) {
                        ...
                        if (stableRefresh || diff >= samples) {
                            ...
                            if (samplesTimeCount > samplesTimeUpdate) {
                                ...
                                MainActivityRecording.showProgress(RecordingService.this, true, phone, samplesTime / sampleRate, true);
                            }
                        }
                    }
    ...
    void encoding(final File in, final Uri uri, final Runnable done, final Success success) {
        ...
        final Runnable save = new Runnable() {
            ...
            public void run() {
                ...
                MainActivityRecording.showProgress(RecordingService.this, false, phone, samplesTime / sampleRate, false);
        ...
        encoder.run(new Runnable() {
            ...
            public void run() {  // progress
                MainActivityRecording.setProgress(RecordingService.this, encoder.getProgress());
            }
        }, new Runnable() {
            @Override
    ...
    void pauseButton() {
        if (thread != null)
            ...
            startRecording();
        MainActivityRecording.showProgress(this, true, phone, samplesTime / sampleRate, thread != null);
    }
